...
class TestProjectFromHome:
    baseURL = ReadConfig.get_application_url(self)
    email = ReadConfig.get_email()
    password = ReadConfig.get_password()

    def test_project_home(self, setup):
        self.browser = setup
        ...
        time.sleep(3)
        self.cm.clickNewProjectTab()
        time.sleep(3)
        self.cm.clickNameInput(self.createProject.projectName)
        time.sleep(3)
        ...
        time.sleep(3)
        self.cm.enterNotes(self.createProject.notes)
        time.sleep(3)
